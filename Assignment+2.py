...
def answer_one():
    ...
    #     print(result)
        X=X_train.reshape(11,1)
        poly=PolynomialFeatures(degree=deg)
        X_poly=poly.fit_transform(X)
        Y=np.linspace(0,10,100).reshape(100,1)
        Y_poly=poly.fit_transform(Y)
        reg=LinearRegression().fit(X_poly,y_train)
        y_pred=reg.predict(Y_poly)
    #     print(y_pred)
        result=np.concatenate((result,y_pred.reshape(1,100)),axis=0)
    result=result[1:,:]
    return result
answer_one()
...
#     print(y_test)
    ...
    deg=range(10)
    for deg in deg:
        X_train_reshaped=X_train.reshape(11,1)
        poly=PolynomialFeatures(degree=deg)
    ...
    result=tuple([np.array(train),np.array(test)])
    return result
...
def answer_three():
    r2=answer_two()
    plt.figure()
    plt.plot(range(10),r2[0],label="train",c="g")
    plt.plot(range(10),r2[1],label="test",c='r')
    plt.xlabel("Degree")
    plt.ylabel("R2")
    plt.xticks(range(10))
    plt.legend(loc='best')
...
def answer_four():
    ...
    reg_test_score=reg.score(X_test_poly,y_test)
    
    lasso=Lasso(alpha=0.01,max_iter=10000).fit(X_train_poly,y_train)
    
    lass_test_score=lasso.score(X_test_poly,y_test)
    
    result=tuple((reg_test_score,lass_test_score))
...
def answer_five():
    ...
    df=pd.DataFrame({'Imp':imp,'colnames':X_train2.columns})
    
    df.sort_values(by='Imp',inplace=True,ascending=False)
